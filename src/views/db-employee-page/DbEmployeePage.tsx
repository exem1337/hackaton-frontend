...
const DbEmployeePage = () => {
  ...
  function deleteSubmit() {
    EmployerService.pathUser({
      ...users[users.findIndex((value) => value.id === activeUser.id)],
      ["status"]: "Уволен",
    });
  }
  ...
  return (
    <div className={"db-employee-page app-container"}>
        ...
        <BaseWrapperSlot>
          {users.map((db_, index) => (
            <div key={index} className={"db-employee-page--list"}>
              <BaseWrapper
                title={`${db_?.last_name} ${db_?.first_name}`}
                smallTitle
              >
                <BaseWrapperSlot>
                  ...
                  <div
                    className={"department-view--themes__theme--inner w-100"}
                  >
                    <Col xs={8}>
                      ...
                      <div className={"db-employee-page--bloc-info"}>
                        <p>
                          ...
                          <span>{db_?.first_name}</span>
                        </p>
                        <p>
                          <span className={"me-2"}>Фамилия :</span>
                          <span>{db_?.last_name}</span>
                        </p>
                        <p>
                          <span className={"me-2"}>Отчество :</span>
                          <span>{db_?.middle_name}</span>
                        </p>
                        <p>
                        ...
                        <p>
                          <span className={"me-2"}>Должность :</span>
                          <span>{db_?.position?.name}</span>
                        </p>
                        <p>
                          <span className={"me-2"}>Номер телефона :</span>
                          <span>{db_?.phone}</span>
                        </p>
                        <p>
                          <span className={"me-2"}>Ставка :</span>
                          <span>{db_?.last_name}</span>
                        </p>
                        <p>
                          <span className={"me-2"}>Статус сотрудника :</span>
                          <span>{db_?.last_name}</span>
                        </p>
                      </div>
                    </Col>
                    <Col>
                      <Col xs={6} md={4} className={""}>
                        <Image
                          className={"db-employee-page--photo"}
                          src={`data:image/png;base64,${
                            userStore?.user?.avatar as unknown as string
                          }`}
                          roundedCircle
                        />
                      </Col>
      ...
      {activeUser?.id !== null && (
        <ModalWindow
          show={activeUser?.id !== null}
          onHide={() => activeUserRestart()}
        >
          {activeUser?.editStatus ? (
            <ModalDbEmployees
              onHandelSaveUsersEdit={onHandelSaveUsersEdit}
              activeUserRestart={activeUserRestart}
              id={activeUser?.id}
            />
          ) : (
            <ConfirmModal
              header={"Увольнение сотрудника"}
              onHide={activeUserRestart}
              onHandel={deleteSubmit}
            >
              <p>
                Вы действительно уверены что хотите уволить -{" "}
                <span>
                  {users[
                    users.findIndex((value) => value?.id === activeUser?.id)
                  ]?.last_name +
                    " " +
                    users[
                      users.findIndex((value) => value?.id === activeUser?.id)
